	...
	<div id="app">
		...
		<div class="conteudo">
			<form v-if="!enviado" class="painel">
				...
				<Rotulo nome="E-mail">
					<input type="text" v-model.lazy.trim="usuario.email"><!--v-model.lazy =>Só atualiza quando o usuário sai do campo; .trim => remove os espaços das pontas-->
				</Rotulo>
				<Rotulo nome="Senha">
					<input type="password" v-model="usuario.senha"> <!-- v-model="usuario.senha" é igual a :value='usuario.senha' @input="usuario.senha = $event.target.value" => equivale ao bind de duas direções-->
				</Rotulo>
				<Rotulo nome="Idade">
					<input type="number" v-model.number="usuario.idade"><!-- .number => Salva o valor como número e não como string-->
				</Rotulo>
				...
				<Rotulo nome="Prioridade">
					<select v-model="prioridade"><!--Se não usarmos o :value="prioridade.codigo" ele usara o {{prioridade.nome}} como chave do select-->
						<option v-for="prioridade in prioridades"
						:key="prioridade.codigo" :value="prioridade.codigo" 
